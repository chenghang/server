...
AsyncLogging::AsyncLogging(const string basename,
  ...
  : flushInterval_(flushInterval),
    running_(false),
    basename_(basename),
    thread_(std::bind(&AsyncLogging::threadFunc, this), "Logging"),//绑定线程函数
    mutex_(),
    cond_(mutex_),
...
void AsyncLogging::append(const char* logline, int len)  //将小缓冲区整理成大的缓冲块
{
...
void AsyncLogging::threadFunc()
{
  ...
  while (running_)
  {
	...
	/*如果在buffers_缓冲队列中的缓存块个数大于25，就把前三个缓存块保存下来，其他都去掉。
